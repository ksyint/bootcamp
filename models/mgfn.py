...
def MSNSD(features,scores,bs,batch_size,drop_out,ncrops,k):
    ...
    number=1920
    normal_features = features[0:number]  # [b/2*ten,32,1024]
    normal_scores = scores[0:batch_size]  # [b/2, 32,1]
    
    abnormal_features = features[number:] #[2880, 32, 1024]
    abnormal_scores = scores[batch_size:]
    #print("features,",features.shape,normal_features.shape,abnormal_features.shape)
    #torch.Size([3840, 32, 1024]) torch.Size([40, 32, 1024]) torch.Size([3800, 32, 1024])
    ...
    abnormal_features = abnormal_features.view(n_size, -1, t, f)
    abnormal_features = abnormal_features.permute(1, 0, 2, 3)

    total_select_abn_feature = torch.zeros(0)
    #print("abnormal",abnormal_features.shape)
    ...
    normal_features = normal_features.view(n_size, -1, t, f)
    normal_features = normal_features.permute(1, 0, 2, 3)
    #print("normal",normal_features.shape)
...
class mgfn(nn.Module):
    ...
    def forward(self, video):
        k = 3
        bs, ncrops, t, c = video.size() #ncrops == frame ?? 
